...
# 配置参数（可自行修改）
# =======================
UPLOAD_URL = 'http://192.168.8.105:5000/upload'  # 上传服务器地址
...
def upload_image(task):
    ...
    try:
        ...
        print(f"[{datetime.now()}] ✅ Uploaded: {task.filename} ({response.status_code})")
        return True
...
def upload_worker(upload_queue):
    ...
    while True:
        ...
        if not upload_image(task) and task.retry < MAX_RETRY:
            task.retry += 1
            print(f"🔁 Retrying {task.filename} (Attempt {task.retry}) in {RETRY_INTERVAL}s...")
